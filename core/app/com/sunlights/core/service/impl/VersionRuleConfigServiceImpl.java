...
public class VersionRuleConfigServiceImpl implements VersionRuleConfigService {
    ...
    public Message checkVersion(String platform, String currentClientVersion) {
        ...
        if(currentClientVersion.compareTo(versionRuleConfig.getMaxSupportVersion()) == 0) {
            message = new Message(MsgCode.CURRENT_LATEST_VERSION, versionRuleConfig.getMaxSupportVersion());
        } else if(currentClientVersion.compareTo(versionRuleConfig.getMinSupportVersion()) < 0) {
            message = new Message(MsgCode.UPDATE_VERSION_TO_CURRENT, versionRuleConfig.getMaxSupportVersion());
        } else if(currentClientVersion.compareTo(versionRuleConfig.getMaxSupportVersion()) > 0){
            String latestVersion = appVersionService.getLatestVersionFromAppStore(platform);
            message = new Message(MsgCode.CURRENT_LATEST_VERSION, latestVersion);
        } else {
            message = new Message(MsgCode.REMIND_UPDATE_VERSION, versionRuleConfig.getMaxSupportVersion());
        }
